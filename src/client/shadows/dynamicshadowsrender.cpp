	...
	if (!m_shadow_node_array.empty()) {
		...
		for (DirectionalLight &light : m_light_list) {
			...
			if (m_shadow_depth_cb) {
				m_shadow_depth_cb->MapRes = (u32)m_shadow_map_texture_size;
		...
		DirectionalLight &light, scene::E_SCENE_NODE_RENDER_PASS pass)
{
	...
	if (m_shadow_map_colored && pass != scene::ESNRP_SOLID) {
		material.MaterialType = m_client->getShaderSource()->getShaderInfo(depth_shader_trans_id).material;
	}
	else {
		material.MaterialType = m_client->getShaderSource()->getShaderInfo(depth_shader_id).material;
		...
		video::ITexture *target, DirectionalLight &light)
{
	...
	for (const auto &shadow_node : m_shadow_node_array) {
		...
		for (u32 m = 0; m < n_node_materials; m++) {
			...
			current_mat.MaterialType = m_client->getShaderSource()->getShaderInfo(depth_shader_entities_id).material;
...
void ShadowRenderer::createShaders()
{
	...
	try {
		...
		depth_shader_entities_id = shdsrc->getShader(name_prefix + "pass1", input_const, video::EMT_SOLID, m_shadow_depth_cb);
		...
		if (m_shadow_map_colored) {
			depth_shader_trans_id = shdsrc->getShader(name_prefix + "pass1_trans", input_const, video::EMT_SOLID, m_shadow_depth_cb);
		}
